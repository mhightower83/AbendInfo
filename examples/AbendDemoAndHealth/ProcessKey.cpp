...
////////////////////////////////////////////////////////////////////////////////
// Empty alloca handler
static IRAM_ATTR
void alloca_handler(
    [[maybe_unused]] struct __exception_frame *ef, [[maybe_unused]] int cause) {
...
void processKey(Print& out, int hotKey) {
  switch (hotKey) {
    ...
    //   break;
    case 'a':
      out.println(F("Install Abend Handler"));
      abendHandlerInstall();
      break;
    case 'r':
    ...
    case 'S':
      out.printf_P(PSTR("Now crashing with Software WDT. This will take about 3 seconds.\r\n"));
      ets_printf("Emulate SDK panic, uses deliberate infinite loop.\n");
      while (true) {}
      break;
    case 'h':
    ...
    case 'H':
      ...
      {
        xt_rsil(15); // Block Soft WDT
        ets_printf("Emulate SDK panic, uses deliberate infinite loop w/Interrupts off.\n");
        while (true) {}
      }
      break;
    case 'p':
    ...
    //   break;
    case '5': {
      _xtos_set_exception_handler(5u, alloca_handler);
      //Exception 5  - AllocaCause - movsp
      printCauseTable();
      out.println(F("Try for an Exception 5  - AllocaCause - movsp ..."));
      uint32_t res, in = 100;
      asm volatile("movsp %[res], %[in]\n\t"
                   : [res]"=&r"(res) : [in]"r"(in): "memory");
      out.printf_P(PSTR("movsp result, res=0x%08x, in=0x%08\r\n"), res, in);
      }
      break;
    case '6': {
        out.println(F("Unaligned Load."));
    ...
    case 'y':
        ...
        break;
    case 'm':
        abendInfoReport(out);     // Results from restart
        abendInfoHeapReport(out); // Current Heap status
    ...
    case '?':
      ...
      out.println(F("  a    - Install Abend Handler"));
      ...
      out.println(F("  5    - Exception 5  - AllocaCause - movsp"));
